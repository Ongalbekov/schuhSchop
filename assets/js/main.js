...
const data = [
     ...
     const schopWaren1=document.querySelector('.warenkorbText');
     let score=0;
function schopwaren(){
        score += 1;
        schopWaren1.innerHTML=score;
     }
     ...
     const toWarenkorb=document.querySelector('.toWarenkorb')
     arrow.addEventListener('click', ()=>{

    

     function creaHTMLElements(){
        for(let i=0; i<data.length; i++){
                let containerDiv=create('div',{className:'toWarenkorb'}, arrow);

                let obj=data[i];
                create('p',{textContent:'name:' + obj.name}, containerDiv);
                create('p', {textContent: 'brand:' + obj.brand}, containerDiv);
                create('p', {textContent: 'sizes:' +obj.sizes},containerDiv);


        }
     }

     function create(type, props, arrow) {
